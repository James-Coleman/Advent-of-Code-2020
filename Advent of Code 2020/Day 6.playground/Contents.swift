...
let exampleGroups = exampleInput.components(separatedBy: "\n\n")

let exampleSets = exampleGroups.map { Set($0) }

let exampleSetsWithoutNewline = exampleGroups.map { group -> Set<String.Element> in
    let withoutNewLine = group.filter { $0 != "\n" }
    return Set(withoutNewLine.map { $0 })
}

exampleSetsWithoutNewline

let exampleCounts = exampleSetsWithoutNewline.map { $0.count }

let exampleSum = exampleCounts.reduce(0, +) // 11 (correct)

func sumOfUniqueAffirmativeAnswers(input: String) -> Int {
    let groups = input.components(separatedBy: "\n\n")
    
    let sets  = groups.map { group -> Set<String.Element> in
        let withoutNewLine = group.filter { $0 != "\n" }
        return Set(withoutNewLine)
    }
    
    let counts = sets.map { $0.count }
...
sumOfUniqueAffirmativeAnswers(input: exampleInput) // Still 11
...
//sumOfUniqueAffirmativeAnswers(input: challengeInput) // 6596 (correct)

// MARK: Part 2 Example

let exampleUnique = exampleGroups.map { group -> Set<String.Element>? in
    let persons = group.split(separator: "\n").map { String($0) }
    let unique = persons.reduce(nil) { (soFar, next) -> Set<String.Element> in
        if let soFar = soFar {
            return soFar.intersection(Set(next))
        } else {
            return Set(next)
        }
    }
...
let exampleUniqueCount = exampleUnique
    .compactMap { $0?.count }
    .reduce(0, +) // 6 (correct)

func sumOfConsensusAffirmativeAnswers(input: String) -> Int {
    let groups = input.components(separatedBy: "\n\n")
    ...
    let counts = sets.compactMap { $0?.count }
...
sumOfConsensusAffirmativeAnswers(input: exampleInput) // Still 6

sumOfConsensusAffirmativeAnswers(input: challengeInput) // 3219 (correct)

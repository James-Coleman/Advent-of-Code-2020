...
struct Password {
    ...
    enum Error: Swift.Error {
        case notEnoughSubstrings(count: Int)
        case minNotInt(string: String)
        case maxNotInt(string: String)
    }
    ...
    init(_ string: String) throws {
        ...
        guard splitString.count == 4 else { throw Error.notEnoughSubstrings(count: splitString.count) }
        
        guard let minInt = Int(splitString[0]) else { throw Error.minNotInt(string: splitString[0]) }
        
        guard let maxInt = Int(splitString[1]) else { throw Error.maxNotInt(string: splitString[1]) }
        
        let rule = PasswordRule(character: splitString[2], min: minInt, max: maxInt)
        
        self.password = splitString[3]
        self.rule = rule
        
        let passwordCharacters = splitString[3].filter { $0 == Character(splitString[2]) }
        
        self.isValid = passwordCharacters.count >= minInt && passwordCharacters.count <= maxInt
    }
}
...
let exampleInputArray = exampleInput.split(separator: "\n").map { String($0) }

do {
    let examplePasswords = try exampleInputArray.map { try Password($0) }
    let validPasswords = examplePasswords.filter { $0.isValid }
    validPasswords.count // 2 (correct)
} catch {
    error
}
...
let challengeInputArray = challengeInput.split(separator: "\n").map { String($0) }

do {
    let challengePasswords = try challengeInputArray.map { try Password($0) }
    let validPasswords = challengePasswords.filter { $0.isValid }
    validPasswords.count // 550 (correct)
} catch {
    error
}
...
struct NewPassword {
    ...
    enum Error: Swift.Error {
        case notEnoughSubstrings(count: Int)
        case firstPositionNotInt(string: String)
        case secondPositionNotInt(string: String)
        case notEnoughPasswordCharacters(count: Int, need: Int)
    }
    ...
    init(_ string: String) throws {
        ...
        guard let firstPositionInt = Int(splitString[0]) else { throw Error.firstPositionNotInt(string: splitString[0]) }
        
        guard let secondPositionInt = Int(splitString[1]) else { throw Error.secondPositionNotInt(string: splitString[1]) }
        
        let requiredCharacter = splitString[2]
        
        let rule = PasswordRule(character: requiredCharacter, firstPosition: firstPositionInt, secondPosition: secondPositionInt)
        
        let password = splitString[3]
        
        self.password = password
        self.rule = rule
        
        guard password.count >= secondPositionInt else { throw Error.notEnoughPasswordCharacters(count: password.count, need: secondPositionInt) }
        
        let firstCharacter = String(password[String.Index(utf16Offset: firstPositionInt - 1, in: password)])
        
        let secondCharacter = String(password[String.Index(utf16Offset: secondPositionInt - 1, in: password)])
        
        self.isValid = (firstCharacter == requiredCharacter || secondCharacter == requiredCharacter) && firstCharacter != secondCharacter
    }
}

do {
    let examplePasswords = try exampleInputArray.map { try NewPassword($0) }
    let validPasswords = examplePasswords.filter { $0.isValid }
    validPasswords.count // 1 (correct)
} catch {
    error
}
...
do {
    let challengePasswords = try challengeInputArray.map { try NewPassword($0) }
    let validPasswords = challengePasswords.filter { $0.isValid }
    validPasswords.count // 634 (correct)
} catch {
    error
}

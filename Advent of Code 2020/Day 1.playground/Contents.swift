...
func sumTo2020(input: [Int]) -> (first: Int, second: Int)? {
    for (firstIndex, firstInt) in input.enumerated() {
        for (secondIndex, secondInt) in input.enumerated() {
            ...
            print("comparing \(firstInt) and \(secondInt)")
...
let intTestInput = intArray(from: testInput)

if let sum = sumTo2020(input: intTestInput) {
    let product = sum.first * sum.second // 514579 (correct)
}
...
if let sum = sumTo2020(input: intChallengeInput) {
    let product = sum.first * sum.second // 41979 (correct)
}
...
if let sum = threeSumTo2020(input: intTestInput) {
    let product = sum.first * sum.second * sum.third // 241861950 (correct)
}

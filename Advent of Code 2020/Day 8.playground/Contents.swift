...
let exampleStringInstructions = exampleInput.components(separatedBy: "\n")
...
do {
    let exampleInstructions = try exampleStringInstructions.map { try Instruction($0) }
    
    var exampleExecutor = InstructionExecutor(instructions: exampleInstructions)
    let exampleAccumulator = try exampleExecutor.accumulatorValuePart1()
} catch {
    error
}
...
do {
    ...
    var challengeExecutor = InstructionExecutor(instructions: challengeInstructions)
//    let challengeAccumulator = try challengeExecutor.accumulatorValuePart1() // 1553 (correct)
} catch {
    error
}
...
do {
    ...
    var exampleExecutor = InstructionExecutor(instructions: exampleInstructions)
    let exampleAccumulator = try exampleExecutor.accumulatorValuePart2() // true, 5 (correct)
} catch {
    error
}
...
do {
    ...
    let examplePermutations = exampleInstructions.repairedPermutations
    
    for permutation in examplePermutations {
        var exampleExecutor = InstructionExecutor(instructions: permutation)
        let exampleAccumulator = try exampleExecutor.accumulatorValuePart2()
    }
} catch {
    error
}
...
do {
    ...
    for permutation in challengePermutations {
        ...
        if challengeAccumulator.infinite == false {
            challengeAccumulator.accumulator
            break
        }
    }
} catch {
